...
func newExactUtxoRange(address common.Address, afterBlock uint32, afterTransaction uint32, afterOutput uint8) (*utxoRange, error) {
	r := &utxoRange{}
	blockNumberBuffer := make([]byte, transaction.BlockNumberLength)
	binary.BigEndian.PutUint32(blockNumberBuffer, afterBlock)
	transactionNumberBuffer := make([]byte, transaction.TransactionNumberLength)
	...
	outputNumberBufferPlusOne := make([]byte, transaction.OutputNumberLength)
	outputNumberBufferPlusOne[0] = afterOutput + 1

	endingKey := []byte{}
	endingKey = append(endingKey, address[:]...)
	endingKey = append(endingKey, blockNumberBuffer...)
	endingKey = append(endingKey, transactionNumberBuffer...)
	endingKey = append(endingKey, outputNumberBufferPlusOne...)
	endingKey = append(endingKey, valueBuffer...)
	r.beginingKey = key
	r.endingKey = endingKey
	return r, nil
}
...
func (r *UTXOlister) GetUTXOsForAddress(address common.Address, afterBlock uint32, afterTransaction uint32, afterOutput uint8, limit int, showWithdrawn bool) ([][transaction.UTXOIndexLength]byte, error) {
	...
	for _, kv := range values {
		...
		if showWithdrawn {
			if len(value) != 1 ||
				value[0] != commonConst.UTXOexistsButNotSpendable ||
				len(key) != expenctedKeyLength {
				continue
			}
		} else {
			if len(value) != 1 ||
				value[0] != commonConst.UTXOisReadyForSpending ||
				len(key) != expenctedKeyLength {
				continue
			}
		}
...
func (r *UTXOlister) GetExactUTXOsForAddress(address common.Address, blockNumber uint32, transactionNumber uint32, outputNumber uint8, limit int, showWithdrawn bool) ([][transaction.UTXOIndexLength]byte, error) {
	options := fdb.RangeOptions{}
	options.Limit = limit
	readingRange, err := newExactUtxoRange(address, blockNumber, transactionNumber, outputNumber)
	if err != nil {
		return nil, err
	}
	pr, err := fdb.PrefixRange([]byte{})
